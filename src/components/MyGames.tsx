...
export default function MyGames({ onResume, onBack }: MyGamesProps) {
  ...
  useEffect(() => {
    if (!player) return;
    const load = async () => {
      // Fetch games where this player is p1 or p2
      const { data } = await supabase
        .from('games')
        .select('id, room_code, player1_id, player2_id, status, state, updated_at')
      ...
      const opponentIds = data.map(g =>
        g.player1_id === player.id ? g.player2_id : g.player1_id
      ).filter(Boolean);
      ...
      const rows: GameRow[] = data.map(g => {
        ...
        return {
          ...
          opponent_name: opponentId ? (nameMap[opponentId] || 'Unknown') : 'Waiting...',
          is_my_turn: currentPlayer === myPlayer,
        };
      });
        ...
        {loading ? (
          ...
          <div className="w-full overflow-y-auto space-y-2">
            {games.map(g => (
              <button
                key={g.id}
                onClick={() => onResume(g.id, g.room_code, g.my_player)}
                className="w-full flex items-center justify-between px-4 py-3 rounded-lg
                           bg-black/20 hover:bg-black/30 transition-colors cursor-pointer text-left"
              >
                <div>
                ...
                <div className="text-white/30 text-xs font-heading">{g.room_code}</div>
              </button>
            ))}
          </div>
        )}
